...
**/
void PlotLog( const TrackHistory& log,
                    IplImage*     img,
                    CvScalar      col,
                    CvRect        rect,
                    int           margin,
...
void TransformLog( const TrackHistory& log, TrackHistory& newlog, const CvMat* H )
{
	...
	for ( unsigned int i=0; i<log.size(); ++i )
	{

		{
            ...
            float angle = norm_angle(log[i].GetOrientation() + asin(cvmGet(H,0,1)));
...
**/
void LogCmToPx( const TrackHistory& in,
                      TrackHistory& out,
                      float         scale,
                      CvPoint2D32f  offset )
{
...
**/
void LogPxToCm( const TrackHistory& in,
                      TrackHistory& out,
                      float         scale,
                      CvPoint2D32f  offset )
{
...
void LogPxToImage( const TrackHistory&      in,
                         TrackHistory&      out,
...
void LogImageToPx( const TrackHistory&      in,
                         TrackHistory&      out,
                   const CameraCalibration* cal,
                   const CvPoint2D32f*      offset )
{
...
void ConvertToRelativeLog( const TrackHistory& in, TrackHistory& out )
{
	...
	if ( in.size() > 0 )
	{
		...
		float cosa = cosf( (D_PI*.5)-oStart );
		float sina = sinf( (D_PI*.5)-oStart );
		...
		for	( unsigned int i=0; i<in.size(); ++i )
		{
			...
			float angle = norm_angle(in[i].GetOrientation() - oStart);
