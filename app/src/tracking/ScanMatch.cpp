...
**/
void scan_association_nearest( const TrackHistory& a1, const TrackHistory& b1, TrackHistory& a2, TrackHistory& b2, float thresh, float timeOffset )
{
...
#ifndef NDEBUG
    // print timestamps
    std::cerr << "TrackHistory a1: "<<std::endl;
    for ( size_t i = 0; i < a1.size(); ++i )
    {
        std::cerr << a1[i].t() << " ";
    }
    std::cerr << "TrackHistory b1: "<<std::endl;
    ...
    for ( unsigned int i=0; i<a1.size(); ++i )
    {
        ...
        if ( e_idx != -1 && l_idx != -1 ) // if we found an earlier AND a later index
        {
                ...
                ) // and they are both within time threshold of each other
            {
                ...
                TrackEntry assoc = Interpolate( b1[e_idx], b1[l_idx], w );
...
**/
ScanPose scan_match( const TrackHistory& a, const TrackHistory& b,
                    TrackHistory& a2, TrackHistory& b2,
                     float thresh, float timeOffset )
{
    TrackHistory a1,am;
    ...
    scan_remove_stationary_points( a, a1, MIN_DIST );
    //scan_remove_bad_points( am, a1, 0.5 );

    scan_remove_stationary_points( b, b1, MIN_DIST );
    //scan_remove_bad_points( bm, b1, 0.5 );

    scan_association_interpolated( a1, b1, a2, b2, thresh, timeOffset );
    //scan_association_nearest( a1, b1, a2, b2, thresh, timeOffset );

    assert( !a2.empty() );
    assert( !b2.empty() );

    ScanPose pose = scan_compute_pose_weighted(a2, b2);
    pose.error = scan_compute_error( pose, a2, b2 );
...
**/
void scan_combine( const TrackHistory& a1, const TrackHistory& b1, double thresh,
                   TrackHistory& avg )
{
    avg.clear();
    ...
    for ( unsigned int i=0; i<a1.size(); ++i )
    {
            ...
            )
        {
            ...
            w = wi / (wa+wi);
            TrackEntry average = Interpolate( a1[i], assoc, w );
...
**/
void scan_merge_and_remove(const TrackHistory& a, const TrackHistory& b, double timeThresh,
                      TrackHistory& m)
{
    m.clear();
...
**/
void scan_average( const TrackHistory& a, const TrackHistory& b, double fps, TrackHistory& avg )
{
    TrackHistory ab, ba;
    scan_combine( a, b, 2.0/fps, ab );
    scan_combine( b, a, 2.0/fps, ba );
    scan_merge_and_remove( ab, ba, 1.0/(fps), avg );
}
...
{
    TrackHistory assocA, assocB;
    scan_association_nearest( A, B, assocA, assocB, thresh, 0.f );
