...
/**
A coverage system must be initialised with the
dimensions of the tracking image.
 **/
CoverageSystem::CoverageSystem( CvSize warpedImageSize ) :
    m_mask( 0 ), m_floorMask( 0 ), m_floorPixels( 0 )
{
    m_mask = cvCreateImage( warpedImageSize, IPL_DEPTH_8U, 1 );
    cvZero( m_mask );
 ...
 * The robot moves in and out of pixels inthe floor plane,
 * and a counter is incremented for each pixel as it is uncovered
 * by the robot.
 *
 ...
 **/
void CoverageSystem::BrushBarUpdate( CvPoint2D32f pl, CvPoint2D32f pr, CvPoint2D32f cl, CvPoint2D32f cr )
{
    cvZero( m_inOutMask );
...
/**
Create a map indicating the number of times the robot has covered
particular areas using different colours.
 **/
void CoverageSystem::CreateColouredMap()
{
    cvZero( m_colMap );

    unsigned char colour[11][3] =
    {
    { 0, 0, 0 },
    { 0, 40, 0 },
    { 0, 80, 0 },
    ...
    int step = m_mask->widthStep;
    for ( int r = 0; r < m_mask->height; ++r )
    {
        char* pMask = m_mask->imageData + (r * step);
        const char* const pEnd = pMask + (m_mask->width * m_mask->nChannels);
...
/**
 * Increment the coverage count for any pixels in m_mask which
...
{
    assert( m_mask->widthStep == m_inOutMask->widthStep );
...
/**
Pass in a tracker and update the coverage mask based on current robot position.

Currently fills in a circle at the location of the base of the robot.
Could be modified to only fill region swept out by brush-bar.

@param tracker A robot tracker whose current position will be used to update the mask.
 **/
...
//    cvCircle( m_mask, pb, radius, cvScalar(255,255,255), CV_FILLED, CV_AA);
//}

/**
Update the coverage mask by directly passing in position and radius.
 **/
...
/**
Render the repeat-coverage map as a transparent overlay on another image
...
/**
Render the coverage mask to another image as a transparent overlay.
@param img The image to render to
 **/
void CoverageSystem::DrawMask( IplImage* img, CvScalar colour ) const
{
    assert( img->width == m_mask->width );
    assert( img->height == m_mask->height );
    ...
    for ( int j = 0; j < h; ++j )
    {
        ...
        const char* pMask = m_mask->imageData + j * m_mask->widthStep;
...
{
    cvSaveImage( file_name, m_mask );
}

/**
 *  Compute how much of the floor area has been covered by robot path
 *  (that we have accumulated in the coverage mask so far).
 *
 *  The percentage of coverage is estimated simply by counting pixels in both masks.
 *  @param floormask A mask (of correct dimensions) which is 255 to indicate floor pixels and 0 for non-floor pixels.
...
/**
Counts the number of pixels which have value 255 in the robot coverage mask.
@return The number of white pixels in the coverage mask.
 **/
unsigned int CoverageSystem::GetCoveredPixelCount() const
{
    return cvCountNonZero( m_mask );
}

/**
Returns the number of white pixels in any single channel image.
...
/**
Load a floor mask image from disk and store internally.
Also compute the number of white pixelsin the mask for later use.
...
bool CoverageSystem::LoadFloorMask( const char* filename )
{
    ...
    if ( mask == 0 )
    {
        LOG_ERROR(QObject::tr("Could not open image '%s'!").arg(filename));
    ...
    if ( m_floorMask->width == m_mask->width &&
         m_floorMask->height == m_mask->height &&
    ...
    else
    {
        ...
        LOG_ERROR(QObject::tr("Expected a %1x%2 grey-scale image!").arg(m_mask->width)
                                                                   .arg(m_mask->height));
        assert( 0 );
        return false;
    }
...
{
    if ( m_floorMask == 0 || m_floorPixels == 0 )
    {
        return -1.f;
    }
...
/**
@brief Returns the percentage of floor space that was covered more than once.
 **/
float CoverageSystem::EstimateRepeatCoverage() const
{
    if ( m_mask == 0 || m_floorPixels == 0 )
    {
        return -1.f;
    }

    return CountRepeatCoverage( m_mask ) * (100.f / m_floorPixels);
}

/**
@brief Computes the area that was covered a particular number of
...
void CoverageSystem::CoverageHistogram( const char* file ) const
{
    ...
    if ( fp == 0 || m_floorMask == 0 || m_floorPixels == 0 )
    {
        return;
    }

    // first make a copy of the coverage mask
    IplImage* mask = cvCloneImage( m_mask );
    IplImage* count = cvCloneImage( mask );

    double t = 1.0;
    for ( int i = 1; i < 256; ++i )
    {
        cvCmpS( m_mask, t, mask, CV_CMP_EQ ); // Find all pixels that were covered 't' times...
        ...
        t += 1.0;
        float pc = nPixels * (100.f / m_floorPixels); // percentage of floor area covered
        fprintf( fp, "%d %f\n", i, pc );
    }
...
void CoverageSystem::WriteIncrementalCoverage( FILE* fp, unsigned int count )
{
    if ( fp )
    {
        IplImage* dst = cvCreateImage( cvSize( m_mask->width,
                                               m_mask->height ), IPL_DEPTH_8U, 1 );
        IplImage* countImage = cvCloneImage( dst );

        for ( unsigned int i = 1; i <= count; ++i )
        {
            cvCmpS( m_mask, i, dst, CV_CMP_GE );
            cvAnd( dst, m_floorMask, countImage ); // then intersect those pixels with floor mask
...
/**
@brief Create a mask which shows areas of the floor that were missed completely.
 **/
