                                       ...
                                       CvPoint2D32f*   offset )
{
    ...
    extractCvHomography(intrinsicMatrix,rotMat,trans,&homMat);
    ...
    invertGroundPlanePoints( inverseCoeffs,
                             intrinsicMatrix,
                             rotMat,
                             trans,
                             4,
    ...
    float margin = 100.f;
    boundingBox( 4, cornerBuffer, &pos, &dim, margin ); // compute bounding box + a margin
    *offset = pos;
    cvFree(&cornerBuffer);
    ...
    *mapy = cvCreateMat( viewWarp->height, viewWarp->width,CV_32F );
    computeGroundPlaneWarp( intrinsicMatrix, rotMat, trans, distortionCoeffs, &pos, *mapx, *mapy );
...
**/
void alignGroundPlane( ScanPose        pose,
                       const IplImage* src,
                       IplImage*       dst,
                       ...
                       CvPoint2D32f    cmpOrigin )
{
    ...
    cv2DRotationMatrix( imgOrigin, -pose.dth*R2D, 1, &affine );
...
**/
void compositeImageBoundingBox( ScanPose        pose,
                                const IplImage* src,
                                ...
                                Rect32f*        bbox )
{
    ...
    CvMat affine = cvMat( 2, 3, CV_32F, A );
    cv2DRotationMatrix( srcOrigin, -pose.dth*R2D, 1, &affine );
    ...
    boundingBox( 8, outVerts, &bbox->pos, &bbox->dim );
